...
$("#button_update_stings").click(function(e) {
	...
	updateSting($("#stingid1").val());
});

$("#button_cerrar_sesion").click(function(e) {
	e.preventDefault();
	cerrarSesion();
});
...
function getStings() {
	...
	$.ajax({
		...
		$.each(stings, function(i, v) {
			...
			$('<li>' +"Stingid:"+ sting.stingid + '</li>').appendTo($grouplist);
			$('<li>' +"Username:"+ sting.username + '</li>').appendTo($grouplist);
			$('<li>' +"Subject:"+ sting.subject + '</li>').appendTo($grouplist);
			$('<li>' +"Content:"+ sting.content + '</li>').appendTo($grouplist);
			$('<li>' +"Timestamp:"+ sting.creationTimestamp + '</li>').appendTo($grouplist);
...
function updateSting(stingid) {
	...
	$.ajax({
		...
		$("#update_result").text("Sting modificadoo correctamente");
	}).fail(function(jqXHR, textStatus) {
...
function cerrarSesion() {
	...
	window.location.replace('http://localhost/beeter/login.html');
}